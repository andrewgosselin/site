...
<template>
    ...
    <PortfolioLayout mainContainerClass="p-0 h-full flex flex-col">
        <!-- Header -->
        <div
            class="hidden shrink-0 flex-row items-center justify-between gap-4 border-b border-black bg-white px-8 py-6 md:flex dark:border-white/10 dark:bg-[#161615]"
        >
            <div class="flex items-center gap-4">
                ...
                <div class="text-xs font-bold tracking-[0.2em] text-gray-600 uppercase">The Hague, NL</div>
            </div>
            <div class="font-mono text-xs font-bold">{{ time }}</div>
        </div>

        <!-- Scrollable Content Area -->
        <div class="flex-1 overflow-y-auto">
            <div class="flex min-h-full flex-col xl:flex-row">
                <!-- Left Column: README -->
                <div class="border-black bg-white xl:w-[400px] xl:border-r 2xl:w-[500px] dark:border-white/10 dark:bg-[#161615]">
                    <div class="p-8">
                        <h2 class="mb-6 text-xs font-bold tracking-[0.3em] text-gray-600 uppercase">About Me</h2>
                        <div
                            v-html="readme"
                            class="prose prose-sm dark:prose-invert prose-headings:font-bold prose-headings:tracking-tight prose-h1:text-2xl prose-h2:text-xl prose-h3:text-lg prose-p:text-sm prose-p:leading-relaxed prose-a:text-blue-600 dark:prose-a:text-blue-400 prose-a:no-underline hover:prose-a:underline prose-ul:text-sm prose-ol:text-sm prose-li:my-1 prose-code:text-xs prose-code:bg-gray-100 dark:prose-code:bg-gray-800 prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-pre:bg-gray-100 dark:prose-pre:bg-gray-800 prose-pre:text-xs max-w-none"
                        ></div>
                    </div>
                </div>

                <!-- Right Column: Bento Grid -->
                <div class="flex-1 bg-[#F5F5F3] p-4 md:p-8 dark:bg-[#101010]">
                    <div class="mx-auto grid max-w-7xl grid-cols-1 gap-4 md:grid-cols-4">
                        <!-- CV (Row 1, Col 1) -->
                        <Link
                            href="/resume"
                            class="group flex flex-col items-center justify-center gap-2 rounded-3xl bg-blue-600 p-8 text-center text-white transition-all hover:bg-blue-500 md:col-span-1"
                        >
                            <i class="pi pi-file text-3xl transition-transform group-hover:-translate-y-1"></i>
                            <span class="text-xs font-bold tracking-[0.4em] uppercase">CV</span>
                        </Link>

                        <!-- Linkedin (Row 1, Col 2) -->
                        <a
                            href="https://www.linkedin.com/in/andrew-gosselin/"
                            target="_blank"
                            class="group flex flex-col items-center justify-center gap-2 rounded-3xl bg-[#0077b5] p-8 text-center text-white transition-all hover:bg-[#006399] md:col-span-1"
                        >
                            <i class="pi pi-linkedin text-3xl transition-transform group-hover:-translate-y-1"></i>
                            <span class="text-xs font-bold tracking-[0.4em] uppercase">Linkedin</span>
                        </a>

                        <!-- Technologies (Top Right, spans 2 cols, 2 rows) -->
                        <div
                            class="flex flex-col rounded-3xl border border-black/5 bg-[#EDEDEC] p-8 md:col-span-2 md:row-span-2 dark:border-white/5 dark:bg-[#1C1C1E]"
                        >
                            <h3 class="mb-6 text-xs font-bold tracking-[0.3em] text-gray-600 uppercase">Commonly Used</h3>
                            <div class="grid flex-1 grid-cols-2 content-start gap-2 text-xs font-bold tracking-widest uppercase sm:grid-cols-2">
                                <a
                                    v-for="t in stackItems"
                                    :key="t.name"
                                    :href="t.url"
                                    target="_blank"
                                    class="flex cursor-pointer items-center gap-2 overflow-hidden rounded-xl border border-black/5 bg-white p-2 transition-transform hover:scale-105 dark:border-white/5 dark:bg-black/20"
                                    :class="t.bg"
                                >
                                    <div
                                    ...
                                    >
                                        ...
                                        <div v-html="t.icon" class="h-4 w-4 fill-current"></div>
                                    </div>
                                    <span class="min-w-0 truncate text-[10px] sm:text-xs">{{ t.name }}</span>
                                </a>
                            </div>
                        </div>
                        ...
                        <!-- Github (Row 2, Col 1) -->
                        <a
                            href="https://github.com/andrewgosselin"
                            target="_blank"
                            class="group flex flex-col items-center justify-center gap-2 rounded-3xl bg-[#181717] p-8 text-center text-white transition-all hover:bg-[#333] md:col-span-1"
                        >
                            <i class="pi pi-github text-3xl transition-transform group-hover:-translate-y-1"></i>
                            <span class="text-xs font-bold tracking-[0.4em] uppercase">Github</span>
                        </a>

                        <!-- Projects (Row 2, Col 2) -->
                        <Link
                            href="/projects"
                            class="group flex flex-col items-center justify-center gap-2 rounded-3xl bg-[#ff5e00] p-8 text-center text-white transition-all hover:bg-[#e65500] md:col-span-1"
                        >
                            <i class="pi pi-th-large text-3xl transition-transform group-hover:-translate-y-1"></i>
                            <span class="text-xs font-bold tracking-[0.4em] uppercase">Projects</span>
                        </Link>

                        <!-- Tools Link (Bottom Left, 2 cols) -->
                        <Link
                            href="/tools"
                            class="group relative flex flex-col justify-between overflow-hidden rounded-3xl bg-black p-8 text-white transition-all hover:translate-y-[-4px] md:col-span-2 dark:bg-white dark:text-black"
                        >
                            <div class="pointer-events-none absolute top-[-20px] right-[-20px] scale-150 rotate-12 transform opacity-10">
                                <i class="pi pi-wrench text-[15rem]"></i>
                            </div>
                            <h3 class="relative z-10 text-4xl font-bold tracking-tighter">DEVELOPER<br />TOOLS</h3>
                            <div class="relative z-10 flex justify-end">
                                <i
                                    class="pi pi-arrow-right transform text-4xl transition-transform group-hover:translate-x-1 group-hover:-translate-y-1"
                                ></i>
                            </div>
                        </Link>
                        ...
                        <Link
                            href="/arcade"
                            class="group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-black bg-[#E5F55F] p-8 text-black transition-all hover:scale-[1.01] md:col-span-2"
                        >
                            <div
                                class="pointer-events-none absolute right-[-20px] bottom-[-40px] scale-150 rotate-[-12deg] transform text-black opacity-10"
                            >
                                <i class="pi pi-bolt text-[15rem]"></i>
                            </div>

                            <div
                                class="relative z-10 mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-black text-[#E5F55F] transition-transform group-hover:rotate-12"
                            >
                                <i class="pi pi-bolt text-2xl"></i>
                            </div>
                            <div class="relative z-10 text-4xl font-bold tracking-tighter">ARCADE</div>
                        </Link>
                    </div>
                </div>
            </div>
